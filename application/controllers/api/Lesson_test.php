...
class Lesson_test extends CI_Controller {
    ...
    public function lessonList(){
        $data =  $this->lesson_model_test->Mlist();
        if(!empty($data)){
    ...
    public function lessonInOut(){
        ...
        if($user_id !='' && $lesson_id!='' && $lesson_status!='' && $user_id !=0 && $lesson_id!=0){
           ...
           if($lesson_status=='in'){
              $checkLessonUserExist = $this->lesson_model_test->insertUserLesson($UserData);
              if($checkLessonUserExist){
               //$this->lesson_model_test->updateUserCount($lesson_id);
             //  $this->lesson_model_test->lessonLog($UserData);
               $message = "Lesson in successfully";
           }else{
            $message = "User already in";
        }
    } else if($lesson_status=='out'){
       $delResult = $this->lesson_model_test->deleteUserFromLesson($lesson_id,$user_id);
       if($delResult){
       // $this->lesson_model_test->updateUserCount($lesson_id);   
       // $this->lesson_model_test->lessonLog($UserData);
...
if($video_status!=''){
    $this->lesson_model_test->updateVideoStatus($video_status,$lesson_id);
}
if($dialog_id!=''){
    $this->lesson_model_test->updateDialogId($dialog_id,$lesson_id);
}
$data = $this->lesson_model_test->getInList($lesson_id);
...
public function getUserCount(){
    ...
    $adminCount = $this->lesson_model_test->MgetAdminCount($lesson_id);
    $studentCount = $this->lesson_model_test->MgetStudentCount($lesson_id);
