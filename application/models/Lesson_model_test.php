...
public function insertUserLesson($UserData){
  return $this->db->insert('user_lessons_test',$UserData);
}
...
public function deleteUserFromLesson($lesson_id,$user_id){
  $this->db->delete('user_lessons_test',array('lesson_id'=>$lesson_id,'user_id'=>$user_id));
...
public function getInList($lesson_id){
 $this->db->select('users.user_id,users.name,user_lessons_test.lesson_id,users.user_type,users.quick_blox_id,user_lessons_test.dialog_id,user_lessons_test.video_status');
 $this->db->from('users');
 $this->db->join('user_lessons_test', 'users.user_id = user_lessons_test.user_id');
 $this->db->join('lessons', 'lessons.lesson_id = user_lessons_test.lesson_id');
 $this->db->where(
  [
    'user_lessons_test.lesson_id' => $lesson_id
  ]);
 $query = $this->db->get();
